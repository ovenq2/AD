...
namespace ADUserManagement.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendPasswordResetConfirmationAsync(string toEmail, string username, string newPassword)
        {
            try
            {
                var subject = "Şifreniz Sıfırlandı - AD Kullanıcı Yönetim Sistemi";
                        ...
                        <div style='max-width: 600px; margin: 0 auto; padding: 20px;'>
                            <div style='background-color: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;'>
                                <h2 style='color: #28a745; margin: 0;'>
                                    <i style='color: #28a745;'>✅</i> Şifreniz Başarıyla Sıfırlandı
                                </h2>
                            </div>
                            
                            <p>Merhaba <strong>{username}</strong>,</p>
                            
                            <p>Sistem yöneticisi tarafından şifreniz sıfırlanmıştır. Yeni giriş bilgileriniz aşağıdaki gibidir:</p>
                            
                            <div style='background-color: #e9ecef; padding: 15px; border-radius: 5px; margin: 20px 0;'>
                                <table style='width: 100%;'>
                                    <tr>
                                        <td style='padding: 8px; font-weight: bold;'>Kullanıcı Adı:</td>
                                        <td style='padding: 8px;'><strong>{username}</strong></td>
                                    </tr>
                                    <tr>
                                        <td style='padding: 8px; font-weight: bold;'>Yeni Şifre:</td>
                                        <td style='padding: 8px; font-family: monospace; background-color: #fff; border: 1px solid #ddd; border-radius: 3px;'>
                                            <strong style='color: #dc3545; font-size: 16px;'>{newPassword}</strong>
                                        </td>
                                    </tr>
                                </table>
                            </div>
                            ...
                            <div style='background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 5px; margin: 20px 0;'>
                                <h4 style='color: #856404; margin: 0 0 10px 0;'>⚠️ ÖNEMLİ GÜVENLİK BİLGİLERİ:</h4>
                                <ul style='color: #856404; margin: 0; padding-left: 20px;'>
                                    <li>İlk giriş yaptığınızda şifrenizi değiştirmeniz istenecektir</li>
                            ...
                            <p>Sisteme giriş yapmak için: <a href='{_configuration["BaseUrl"]}/Account/Login'>{_configuration["BaseUrl"]}/Account/Login</a></p>
                            
                            <p>Herhangi bir sorunuz varsa IT Destek ekibi ile iletişime geçebilirsiniz.</p>
                            
                            <hr style='margin: 30px 0; border: none; border-top: 1px solid #dee2e6;'>
                            <p style='font-size: 12px; color: #6c757d;'>
                                Bu e-posta AD Kullanıcı Yönetim Sistemi tarafından otomatik olarak gönderilmiştir.<br>
                                Bu e-postayı güvenlik nedeniyle lütfen silin veya güvenli bir yerde saklayın.
                            </p>
                        </div>
                    </body>
                    </html>";
                ...
                var emailSent = await SendEmailAsync(emailDto);

                if (emailSent)
                {
                    _logger.LogInformation($"Password reset confirmation email sent to {username} at {toEmail}");
                }
                else
                {
                    _logger.LogWarning($"Failed to send password reset confirmation email to {username} at {toEmail}");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error sending password reset confirmation email to {username} at {toEmail}");
                throw;
            }
        }
    }
